...
import { ACTION_VIEW, LS_INV_SELECTED_DELIVERY_ORDER } from '../../../../constants';
...
export class DobalikComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  ngOnInit(): void {
    this.dtOptions = {
      ...
      columns: [
        ...
        { data: 'TGL_PESANAN', title: 'Tanggal Pesanan' },
        ...
        { data: 'STATUS_DO_BALIK', title: 'Status DO Balik' },
        {
          ...
          render: () => {
            return `<div class="d-flex px-2 gap-2"><button style="width: 100px" class="btn btn-sm action-posting btn-outline-success btn-60 pe-1">Posting</button> <button style="width: 100px" class="btn btn-sm action-view btn-outline-info btn-60">Lihat</button></div>  `;
          },
        },
      ],
    ...
    actionBtnClick(action: string, data: any): void {
      if (action === ACTION_VIEW) {
        ...
        this.router.navigate(['/transaction/delivery-item/dobalik/detail-report-do-balik']); this
      } if (action === 'POSTING') {
        this.g.saveLocalstorage(
          LS_INV_SELECTED_DELIVERY_ORDER,
          JSON.stringify(data)
        ...
        const param = {
          ...
          userPosted : JSON.parse(localStorage.getItem('inv_currentUser') || '').namaUser, 
        }
  ...
  getProsesDoBalik(callback: any): void {
    ...
    const params = {
      ...
      statusPosting: 'I',
      fromDate: formattedStartDate,  // Kirim dengan format 'dd MM yyyy'
      toDate: formattedEndDate, // Jika perlu, bisa dikirim juga
    };

    console.log("Mengirim data ke backend:", params);

    this.dataService
      .postData(this.config.BASE_URL + '/delivery-order/proses-do-balik', params)
      .subscribe(
        (response: any) => {
