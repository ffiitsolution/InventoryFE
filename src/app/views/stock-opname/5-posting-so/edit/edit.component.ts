...
import { DEFAULT_DELAY_TIME, LS_INV_SELECTED_UOM } from 'src/constants';
...
export class TableUomEditComponent implements OnInit {
  myForm: FormGroup;
  ...
  ngOnInit(): void {
    ...
    this.myForm = this.form.group({
      code: [
        { value: this.detail.kodeUom, disabled: true },
        [Validators.required, code],
      ],
      desc: [this.detail.keteranganUom, [Validators.required, desc]],
  ...
  onSubmit(): void {
    ...
    if (invalid) {
      ...
      this.service.insert('/api/uom/update', param).subscribe({
        next: (res) => {
          if (!res.success) {
