...
@Component({
  selector: 'app-edit-regional',
  templateUrl: './edit.component.html',
  styleUrl: './edit.component.scss',
})
export class TableRegionalEditComponent implements OnInit {
  myForm: FormGroup;
  ...
  ngOnInit(): void {
    ...
    this.myForm = this.form.group({
      code: [
        { value: this.detail.kodeRegion, disabled: true },
        [Validators.required, code],
      ],
      desc: [this.detail.keteranganRegion, [Validators.required, desc]],
  ...
  onSubmit(): void {
    ...
    if (invalid) {
      ...
      this.service.insert('/api/region/update', param).subscribe({
        next: (res) => {
          if (!res.success) {
            alert(res.message);
          } else {
